	...
	class MTL_Contact_Bar_Output {
		...
		public function contact_bar_output() {
			...
			if ( $this->facebook_url || $this->twitter_url || $this->linkedin_url || $this->youtube_url || $this->instagram_url ) {
				echo '<div class="text-left-lg text-left-md text-left-sm text-center-xs social-links-container" style="margin-left: 1323px;">';
				echo '<ul class="social-link-list">';

				if ( $this->facebook_url ) {
					echo '<li class="regina-facebook"><a href="https://www.facebook.com/mdentclinic/" title="' . __( 'Facebook', 'regina-lite' ) . '"><span class="nc-icon-glyph socials-1_logo-facebook"></span></a></li>';
				}
				// if ( $this->twitter_url ) {
			...
			if ( $this->phone_number || $this->email_addr || $this->show_search_in_header ) {
				...
				if ( $this->show_search_in_header ) {
					...
					echo '<button class="icon ">Որոնել</button>';
					echo '</form>';
					echo '</div>';
				...
				if ( $this->phone_number ) {
					echo '<p class="regina-phone"><span class="nc-icon-glyph tech_mobile-button"></span>+(374)99 707272</p>';
				}
				if ( $this->email_addr ) {
					echo '<p class="regina-email"><span class="nc-icon-glyph ui-1_email-83"></span><a href="mailto: ' . sanitize_email( $this->email_addr ) . '">mdent@gmail.com</a></p>';
				}
				echo '</div><!--/.col-sm-7-->';
